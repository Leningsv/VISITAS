...
public partial class Policias_frmmanpolEdit : System.Web.UI.Page
{
    #region Variables
    ...
    protected void btngrabar_Click(object sender, ImageClickEventArgs e)
    {
        ...
        if (txtFechaIng.Text != "" || txtFechaSal.Text != "")
        {
            ...
            if (Convert.ToDateTime(txtFechaSal.Text) < DateTime.Now)
            {
        ...
        HabilitarPolicia(Request["CodPol"].ToString(), chkestado.Checked, txtFechaSal.Text);

        Response.Redirect("frmmanpolAdmin.aspx?mensajeRetornado='Guardado con Éxito'");
    }
    ...
    {
        Response.Redirect("frmmanpolAdmin.aspx");
    }
    ...
    #region Funciones y Procedimientos
    private void HabilitarPolicia(string codPolicia, bool Habilitado, string fechaSalida)
    {
        Array.Resize(ref objparam, 4);
        objparam[0] = codPolicia;
        objparam[1] = fechaSalida;
        objparam[2] = Habilitado;
        objparam[3] = Session["usuCodigo"].ToString();
        dsData = fun.consultarDatos("spHabilitarPolicia", objparam, Page, (String[])Session["constrring"]);
    }
