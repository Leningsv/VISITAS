...
public partial class Sancion_frmsanDesbloAdmin : System.Web.UI.Page
{
    #region Variables
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                lbltitulo.Text = "lista de visitantes con sanciones";
                funCargarpost();
    ...
    {
        objparam[0] = 0;
        dsData = fun.consultarDatos("spCarVisitaSancionado", objparam, Page, (String[])Session["constrring"]);
    ...
    protected void btnselect_Click(object sender, ImageClickEventArgs e)
    {
        ...
        ScriptManager.RegisterStartupScript(this.UpdatePanel1, GetType(), "Desbloquear", "javascript: var posicion_x; var posicion_y; posicion_x=(screen.width/2)-(900/2); posicion_y=(screen.height/2)-(600/2); window.open('frmsancionDesbloAplicar.aspx',null,'left=' + posicion_x + ', top=' + posicion_y + ', width=1024px, height=600px, status=no,resizable= yes, scrollbars=yes, toolbar=no, location=no, menubar=no');", true);
    }
    #endregion
}