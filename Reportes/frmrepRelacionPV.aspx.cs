...
public partial class Reportes_frmrepRelacionPV : System.Web.UI.Page
{
    #region Variables
    ...
    protected void btProcesar_Click(object sender, ImageClickEventArgs e)
    {
        try
        {
            ...
            dsData = fun.consultarDatos("spRepRelacionPV", objparam, Page, (String[])Session["constrring"]);
    ...
    {
        dsrepRelacionPV.dtrepRelacionPVDataTable dtReporte1 = new dsrepRelacionPV.dtrepRelacionPVDataTable();
        int intCont = 0;
        while (intCont < DSconsulta.Tables[0].Rows.Count)
        {
            dsrepRelacionPV.dtrepRelacionPVRow row = dtReporte1.NewdtrepRelacionPVRow();
            row.nombrePPL = DSconsulta.Tables[0].Rows[intCont][0].ToString();
            row.etapa = DSconsulta.Tables[0].Rows[intCont][1].ToString();
            row.pabellon = DSconsulta.Tables[0].Rows[intCont][2].ToString();
            row.nroDoc = DSconsulta.Tables[0].Rows[intCont][3].ToString();
            row.nombreVisitante = DSconsulta.Tables[0].Rows[intCont][4].ToString();
            row.tipoVisita = DSconsulta.Tables[0].Rows[intCont][5].ToString();
            row.parentesco = DSconsulta.Tables[0].Rows[intCont][6].ToString();
