...
public partial class TareaN_frmtarAdmin : System.Web.UI.Page
{
    #region Variables
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                lbltitulo.Text = "Administrador de Tareas";
    ...
    protected void funCargaMantenimiento()
    {
        ...
        objparam[0] = 0;
        dsData = fun.consultarDatos("spnTareaAdminReadAll", objparam, Page, (String[])Session["constrring"]);
    ...
    {
        ScriptManager.RegisterStartupScript(this, GetType(), "Tarea_Nueva", "javascript: var posicion_x; var posicion_y; posicion_x=(screen.width/2)-(900/2); posicion_y=(screen.height/2)-(600/2); window.open('frmtarNew.aspx" + "',null,'left=' + posicion_x + ', top=' + posicion_y + ', width=1024px, height=500px, status=no,resizable= yes, scrollbars=yes, toolbar=no, location=no, menubar=no,titlebar=0');", true);
    }
    ...
    protected void btnselecc_Click(object sender, ImageClickEventArgs e)
    {
        ...
        ScriptManager.RegisterStartupScript(this.UpdatePanel1, GetType(), "Editar_Tarea", "javascript: var posicion_x; var posicion_y; posicion_x=(screen.width/2)-(900/2); posicion_y=(screen.height/2)-(600/2); window.open('frmtarEdit.aspx?codigo=" + strCodigoUsu + "',null,'left=' + posicion_x + ', top=' + posicion_y + ', width=1024px, height=500px, status=no,resizable= yes, scrollbars=yes, toolbar=no, location=no, menubar=no,titlebar=0');", true);
    }
    #endregion
}