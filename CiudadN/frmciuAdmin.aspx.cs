...
public partial class CiudadN_frmciuAdmin : System.Web.UI.Page
{
    #region Variables
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                lbltitulo.Text = "Administrador de Ciudades";
    ...
    protected void funCargaMantenimiento()
    {
        ...
        objparam[0] = 0;
        dsData = fun.consultarDatos("spCarCiudadAdmin", objparam, Page, (String[])Session["constrring"]);
    ...
    {
        ScriptManager.RegisterStartupScript(this, GetType(), "Ciudad_Nueva", "javascript: var posicion_x; var posicion_y; posicion_x=(screen.width/2)-(900/2); posicion_y=(screen.height/2)-(600/2); window.open('frmciuNew.aspx" + "',null,'left=' + posicion_x + ', top=' + posicion_y + ', width=1024px, height=500px, status=no,resizable= yes, scrollbars=yes, toolbar=no, location=no, menubar=no,titlebar=0');", true);
    }
    ...
    protected void btnselecc_Click(object sender, ImageClickEventArgs e)
    {
        ...
        String strCodigoCiu = grdvDatos.DataKeys[intIndex].Values["Codigo"].ToString();
        ScriptManager.RegisterStartupScript(this.uppbuscagrilla, GetType(), "Editar_Ciudad", "javascript: var posicion_x; var posicion_y; posicion_x=(screen.width/2)-(900/2); posicion_y=(screen.height/2)-(600/2); window.open('frmciuEdit.aspx?ciu_Codigo=" + strCodigoCiu + "',null,'left=' + posicion_x + ', top=' + posicion_y + ', width=1024px, height=500px, status=no,resizable= yes, scrollbars=yes, toolbar=no, location=no, menubar=no,titlebar=0');", true);
    }
    #endregion
