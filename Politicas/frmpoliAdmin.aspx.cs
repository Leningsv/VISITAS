...
public partial class Politicas_frmpoliAdmin : System.Web.UI.Page
{
    #region variables
    ...
    protected void Page_Load(object sender, EventArgs e)
    {
        try
        {
            if (!IsPostBack)
            {
                lbltitulo.Text = "Administrador de Políticas";
                funCargarpost();
    ...
    {
        objparam[0] = 0;
        dsData = fun.consultarDatos("spnPoliticasAdminReadAll", objparam, Page, (String[])Session["constrring"]);
    ...
    {
        ScriptManager.RegisterStartupScript(this, GetType(), "Política_Nueva", "javascript: var posicion_x; var posicion_y; posicion_x=(screen.width/2)-(1024/2); posicion_y=(screen.height/2)-(600/2); window.open('frmpoliNew.aspx" + "',null,'left=' + posicion_x + ', top=' + posicion_y + ', width=1024px, height=500px, status=no,resizable= yes, scrollbars=yes, toolbar=no, location=no, menubar=no,titlebar=0');", true);
    }
    ...
    protected void btnselecc_Click(object sender, ImageClickEventArgs e)
    {
        ...
        String strCodigoPara = grdvDatos.DataKeys[intIndex].Values["Codigo"].ToString();        
        Response.Redirect("frmpoliEdit.aspx?paraCodigo=" + strCodigoPara);
    }
    #endregion
